...
import {FaCopy, FaSearch, FaPlus, FaPencil, FaDatabase, FaClose, FaExternalLinkSquare, FaPrint} from 'react-icons/lib/fa';
...
export default class ToolModelRecords extends Component {
	constructor(props) {
		super(props);
		this.state = {
      currentCategory: '',
  ...
  loadPage(pagenum) {
    this.setState({page: pagenum, rowData: []}, () => {
      ...
      request
        .get('http://localhost:3001/toolmodels')
  ...
  render() {
    const {sidemenu: {tool_model_records}} = this.props;
    const {page, pageSize, rowData, columnDefs, total} = this.state;
  	return (
  		<div className="tool-model-records fit-screen">
  			<div className="toolbar" style={{paddingLeft: '10px'}}>
  				<a className="btn bg-white black"><FaPlus size={20} style={{color: 'darkgreen'}} /> <span className="vertical-center">Add Model Record</span></a>
  				<a className="btn bg-white black"><FaCopy size={17} style={{color: 'darkblue'}} /> <span className="vertical-center">Copy Model Record</span></a>
  				<a className="btn bg-white black"><FaPencil size={20} style={{color: 'orange'}} /> <span className="vertical-center">Edit Model Record</span></a>
  				<a className="btn bg-white black"><FaClose size={22} style={{color: 'red'}} /> <span className="vertical-center">Delete Model Record</span></a>
  				<a className="btn bg-white black"><FaPlus size={20} style={{color: 'darkgreen'}} /> <span className="vertical-center">Create New Tool</span></a>
          ...
          <a className="btn bg-white black"><FaPrint size={20} /> <span className="vertical-center">Print</span></a>  			
  			...
  			<div className="content">
                ...
                {this.renderBrowserCategories(tool_model_records, 'tool-model-records')}
              </Tree>
  					</div>

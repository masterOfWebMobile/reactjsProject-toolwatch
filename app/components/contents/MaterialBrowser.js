...
import {FaList, FaPlus, FaPencil, FaExternalLinkSquare, FaPrint} from 'react-icons/lib/fa';
...
export default class MaterialBrowser extends Component {
	constructor(props) {
		super(props);
		this.state = {
      currentCategory: '',
      columnDefs: [
        ...
        {headerName: 'Stock Location', field: 'location'},
  ...
  loadPage(pagenum) {
    this.setState({page: pagenum, rowData: []}, () => {
      ...
      request
        .get('http://localhost:3001/materialbrowser')
  ...
  render() {
    const {sidemenu: {material_browser_categories}} = this.props;
    const {page, pageSize, rowData, columnDefs, total} = this.state;
  	return (
  		<div className="material-browser fit-screen">
  			<div className="toolbar">
  				<a className="btn bg-white black"><FaPlus size={20} style={{color: 'darkgreen'}} /> Add Item</a>
  				<a className="btn bg-white black"><FaPencil size={20} style={{color: 'orange'}} /> Edit Item</a>
  				...
  				<div className="view" style={{flex: 0.2}}>
  					...
  					<div className="view-content">
  						<Tree defaultExpandAll={false} showIcon={false} onSelect={this.handleCategoryChange} selectedKeys={[this.state.currentCategory]}>
  							{this.renderBrowserCategories(material_browser_categories, 'material-browser-categories')}
  						</Tree>
  					</div>
              ...
              <button onClick={this.loadPage.bind(this, 0)}>first</button>
              <button onClick={this.loadPage.bind(this, page - 1)}>prev</button>
              <p>{`${page * pageSize + 1} to ${(page + 1) * pageSize} of ${total ? total : 'more'}`}</p>
              <button onClick={this.loadPage.bind(this, page + 1)}>next</button>
              <button onClick={this.loadPage.bind(this, total / pageSize - 1)}>last</button>
            </div>
